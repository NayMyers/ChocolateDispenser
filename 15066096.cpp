...
/////////////  Generic 
class State //This is the super class State. It is generic and contains what is common to all states.  The purpose of this is to be able to create a framework to build the rest of classes 
{ //and to be able to easily make other finite state machines. 
protected:
...
private:
	int inventory = 0; // number of chocolate bars
	int credit = 0; // a measure of the number of bars that can be purchased and not money 
	int pin = 54321; //Variable to store the pin so the enter pin method can compare to this to see if the user entered pin is correct
public:
...
bool HasCredit::moneyRejected(void) //This method sets the context to No_Credit and makes the credit equal to zero this method simulates a sensor being triggered in the machine that would indicate the money
{ //is not valid. Or perhaps a return money button on the outisde of the machine. 
	cout << "Rejecting money...." << endl;
	((Chocolate_Dispenser*)CurrentContext)->credit = 0;
	CurrentContext->setState(No_Credit);
	return true;
}
bool MaintenanceMode::addChocolate(int number) //This method updates the inventory by adding the integer passed into the method onto the inventory variable. 
{
...
bool DispensesChocolate::dispense(void) //This method displayes the current inventory and credit. and deals with the logic of which state to change to based on the inventory and credit 
{ //This method does not change any variables as this is conducted by the makeSelection method
...
bool OutOfChocolate::moneyRejected(void) //This method simply remains at the same state and makes the credit value equal to zero. 
{
	cout << "Rejecting money...." << endl;
	((Chocolate_Dispenser*)CurrentContext)->credit = 0;
	CurrentContext->setState(Out_Of_Chocolate);
...
{  //This method deals with the logic of checking if the pin is correct by comparing the pin stored in the context class (Chocolate_Dispenser) and the integer passed into the method. 
	if (pin == ((Chocolate_Dispenser*)CurrentContext)->pin)
	{
		((Chocolate_Dispenser*)CurrentContext)->setState(Maintinence_Mode);
	}
